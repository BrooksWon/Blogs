...
延迟存储属性注意点：**当结构体包含一个延迟存储属性时，只有var才能访问延迟存储属性；因为延迟存储属性初始化时需要改变结构体的内存。**
...
- 第1阶段：初始化所有存储属性。
  ...
  4. 指定初始化器调用父类的初始化器，不断向上调用，形成初始化链。
...
- 指定初始化器必须保证在调用父类指定初始化器之前，其所在类定义的所有存储属性都要初始化完成。
...
②如果子类提供了父类所有指定初始化器的实现（要么通过方式①继承，要么重写），子类自动继承所有父类的便捷初始化器。
...
要注意的点：不允许同时定义参数标签、参数个数、参数类型相同的可失败初始化器和非可失败初始化器。

- 可以使用 **init！**定义隐士式解包的可失败初始化器。
...
调用时机：当类的实例对象被释放内存时，就会调用实例对象的deinit方法。
...
让枚举遵守CaseInterable协议，可以实现遍历枚举。这个是标准库里的协议。
...
open：允许在定义实体的模块、其他模块访问，允许其他模块进行继承、重写（open只能用在类、类成员上）。
...
- 无主引用（unowned reference）：通过**unowned**定义无主引用。
  - 不会产生强引用，实例销毁后仍然存储着实例的内存地址（类似与OC中的 **unsafe_unretained**）。
